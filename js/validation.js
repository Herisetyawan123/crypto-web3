  ...
  if (registerForm) {
    registerForm.addEventListener('submit', function(event) {
      ...
      if (!validatePassword(password.value)) {
        showError(password, 'Password must be at least 8 characters with at least one number and one uppercase letter');
        isValid = false;
      }
...
function clearErrors() {
  const errorInputs = document.querySelectorAll('.form-input.error, .form-checkbox input.error');
